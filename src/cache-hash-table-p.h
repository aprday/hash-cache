...
class CacheHashTable {
 public:
  ...
  void GetSize(){
	  ...
	  cout<<sizeof(BuildTuple)<<endl;
	  cout<<sizeof(BuildTuple)*CONTENT_SIZE<<endl;
	  cout<<sizeof(j)<<endl;
 ...
 private:
  ...
  static const int CONTENT_SIZE = (LINES_PER_BLOB * LINE_SIZE-sizeof(ContentBlob *)) / sizeof(BuildTuple);  //�������ݵĸ���
  ...
  struct Bucket {
    ...
    ContentBlob *last;//build a index to a ContentBlob
  ...
  idx_t AllocateContent(Bucket* bucket);
